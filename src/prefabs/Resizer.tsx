...
function getShape(
    ...
    const unrotatedPointerPos = rotateCoord(pointerPos, -rotation);
    const allowHorizontalScale = activeHandleId != HandleId.Top && activeHandleId != HandleId.Bottom;
    const allowVerticalScale = activeHandleId != HandleId.Left && activeHandleId != HandleId.Right;
    ...
    if (keepRatio) {
        ...
        const adjustedSizes = {
            width: originalRatio * Math.abs(newSize.height) * Math.sign(newSize.width),
            height: (Math.abs(newSize.width) / originalRatio) * Math.sign(newSize.height),
        };
        ...
        if (!allowHorizontalScale) {
            newSize.width = adjustedSizes.width;
        } else if (!allowVerticalScale) {
            newSize.height = adjustedSizes.height;
        } else {
            // For corner handles, take a weighted average based on the original shape.
            // This should prevent small changes in the "short" direction yielding humongous rectangles
            // when the object is very thin.
            const w = object.width < object.height ? originalRatio : 1 - 1 / originalRatio;
            newSize.height = (newSize.height * (1 - w) + adjustedSizes.height * w) / 1;
            newSize.width = (newSize.width * w + adjustedSizes.width * (1 - w)) / 1;
        }
